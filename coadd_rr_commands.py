...
output_dir    = '/global/cscratch1/sd/mjwilson/DESILBGSPEC/{}/deep/'.format(tileid)
...
nights        = np.array([date])
tiles         = np.array([tileid])

petals        = [0,1,2,3,4,5,7,8,9]
...
if (not os.path.isfile(output_dir + '/allcframes_{}.fits'.format(date))):
        ...
        if len(cframe_list_tmp) > 0:
            if tiles is None:
                ...
                with fitsio.FITS(cframe_list_tmp[0]) as f:
                    if np.int(f[0].read_header()['TILEID']) in tiles:
                        cframe_list += cframe_list_tmp
    ...
    cframes.write(output_dir + '/allcframes_{}.fits'.format(date), format='fits', overwrite=True)

    print('Written: {}'.format(output_dir + '/allcframes_{}.fits'.format(date)))
    
    exit(0)
   
## 
cframes = Table.read(output_dir + '/allcframes_{}.fits'.format(date))
...
if not ALL:
  ...
  if (not overwrite) and (os.path.isfile(output_dir + "/scripts/commands_spectra_deep_{}.sh".format(night)) | os.path.isfile(output_dir + "/scripts/commands_rr_deep_{}.sh".format(night))):
    raise  ValueError('Overwrite=True required to remove exisiting files.')
  
  output_file   = open(output_dir + "/scripts/commands_spectra_deep_{}.sh".format(night), "w")
  output_rrfile = open(output_dir + "/scripts/commands_rr_deep_{}.sh".format(night),    "w")
    ...
    for petal_loc in petals: 
        ...
        for subset_index in range(len(subset_split)):
            ...
            if (not ALL) & (n_exp == 1):
                ...
                output_argument = os.path.join(output_dir, '', 'spectra-{}-{}-deep.fits'.format(petal_loc, tileid))
            ...
            output_file.write('desi_coadd_spectra -i {} --outfile {}\n'.format(output_argument, output_argument.replace('spectra', 'coadd')))
